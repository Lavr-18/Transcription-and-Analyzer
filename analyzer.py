...
PROMPT_TEMPLATE = '''Ты строгий, но справедливый эксперт по продажам, анализирующий телефонные звонки. Твоя задача — внимательно проанализировать диалог между Менеджером и Клиентом и выставить максимально точную оценку по каждому из 13 критериев из чек-листа ОКК.
...
* **-1 (не выполнено):** Критерий был необходим, но менеджер его не выполнил, выполнил плохо, проигнорировал, допустил ошибку или упустил очевидную возможность.
...
2.  **Установление контакта:**
    * **1 (Выполнено):** Менеджер активно стремится установить контакт, используя как минимум ДВА из следующих пунктов: называет клиента по имени (если это уместно и доступно), делает уместный комплимент (профессионализму клиента, настроению), выражает подтверждение правильности выбора клиента (например, "Рады вас слышать!", "Вы сделали правильный выбор, обратившись к нам!").
    * **0 (Не применимо):** Короткий звонок или контекст не предполагал такого взаимодействия.
    * **-1 (Не выполнено):** Менеджер сразу переходит к делу без приветствия, не использовал имя клиента, или приветствие было сухим/формальным, не используя никаких пунктов для установления контакта.
...
8.  **Отработка возражения:** (Этот критерий актуален, только если было возражение)
    * **1 (Выполнено):** Менеджер активно и эффективно отработал возражение, применив формулу работы с возражениями (как минимум ДВЕ ПОПЫТКИ отработки, каждая включает: 1) Уточняющий вопрос, 2) Аргумент, 3) Закрывающий вопрос). По итогу диалога возражение снято или удалось продвинуться дальше по сделке.
...
Также, если Менеджер представился, укажи его имя отдельным полем "manager_name" в конце JSON-ответа. **Используй только имена из предоставленного списка. Это очень важно для корректной аналитики.**
...
**Формат ответа:**
Строго JSON формат. Только числа -1, 0 или 1 для каждого критерия, плюс строка для имени менеджера. Никаких дополнительных комментариев или текста до или после JSON.
...
def clean_json_string(raw_content):
    """
    Извлекает JSON-строку из ответа модели, обрезая лишний текст.
    """
...
def extract_manager_name(text):
    """
    Извлекает имя менеджера из текста транскрипта.
    Используется как запасной вариант, если модель не указала имя.
    """
    # Ищем фразу "Менеджер: ... меня зовут [Имя]" или похожие конструкции
...
def analyze_transcripts(transcripts_folder):
    ...
    """
    # Формируем путь для папки с результатами анализа
    output_folder = transcripts_folder.replace("transcripts", "analyses")
    os.makedirs(output_folder, exist_ok=True)  # Создаем папку, если ее нет

    # Итерируем по всем файлам в папке транскриптов
    for i, filename in enumerate(sorted(os.listdir(transcripts_folder))):
        if not filename.endswith(".txt"):
            continue  # Пропускаем файлы, не являющиеся текстовыми транскриптами

        path = os.path.join(transcripts_folder, filename)
        with open(path, "r", encoding="utf-8") as f:
            transcript = f.read()  # Читаем содержимое транскрипта

        # Формируем полный промпт для модели
        prompt = PROMPT_TEMPLATE.format(transcript=transcript)

        success = False
        # Попытки отправки запроса в OpenAI API (до 3 раз)
        for attempt in range(3):
            try:
                ...
                response = client.chat.completions.create(
                    ...
                    temperature=0,  # Низкая температура для детерминированного ответа (JSON)
                )
                raw_content = response.choices[0].message.content
                json_str = clean_json_string(raw_content)  # Извлекаем только JSON
                result_dict = json.loads(json_str)  # Парсим JSON

                # Фильтруем результат, оставляя только заданные критерии,
                # и используем 0 как значение по умолчанию, если критерий отсутствует
                filtered_result = {key: result_dict.get(key, 0) for key in CRITERIA}

                # Получаем имя менеджера из ответа модели,
                # или из транскрипта, если модель не указала (запасной вариант)
                filtered_result["manager_name"] = result_dict.get("manager_name", extract_manager_name(transcript))
                success = True  # Запрос успешно обработан
                break  # Выходим из цикла попыток
            except Exception as e:
                print(f"⚠️ Ошибка при генерации/парсинге для {filename} (попытка {attempt + 1}): {e}")
                time.sleep(2)  # Пауза перед следующей попыткой

        if not success:
            # Если все попытки неудачны, сохраняем исходный транскрипт как "сырой" результат
...
if __name__ == "__main__":
    # Запуск анализа для транскриптов за текущий день (для тестирования)
    # В основном скрипте main.py вызывается для вчерашнего дня
