...
Также, если Менеджер представился, укажи его имя отдельным полем "manager_name" в конце JSON-ответа.
Если имени нет, укажи "Неизвестно".

Ответ:
JSON формат, только числа -1, 0 или 1, плюс строка для имени менеджера:
...
def extract_manager_name(text):
    match = re.search(r'Менеджер:.*?(?:меня зовут|зовут меня|это)\s+([А-ЯЁ][а-яё]+)', text, flags=re.IGNORECASE)
    ...
    for i, filename in enumerate(sorted(os.listdir(transcripts_folder))):
        ...
        for attempt in range(3):
            try:
                ...
                filtered_result["manager_name"] = result_dict.get("manager_name", extract_manager_name(transcript))
                success = True
                break
